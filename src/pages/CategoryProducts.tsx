...
import { Product, Category, getCategories, getProducts, addProduct, updateProduct, deleteProduct, uploadImage } from '../db';
...
export function CategoryProducts() {
  ...
  const loadData = useCallback(async () => {
    const [cats, prods] = await Promise.all([getCategories(), getProducts(categoryId)]);
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center gap-4 mb-6">
        ...
        {!isAdding && (
          <button onClick={() => setIsAdding(true)} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-blue-700 transition">
            <Plus size={20} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="مثال: هاتف ذكي..." />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">الكمية</label>
              <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value ? Number(e.target.value) : '')}
                className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0" />
            </div>
            <div>
              ...
              <input type="number" value={wholesalePrice} onChange={(e) => setWholesalePrice(e.target.value ? Number(e.target.value) : '')}
                className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="0.00" />
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex gap-2">
                <input type="text" value={barcode} onChange={(e) => setBarcode(e.target.value)}
                  className="flex-1 p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="أدخل الباركود يدوياً أو استخدم الكاميرا" />
              ...
              {isScanning && (
                <div className="mt-4 border rounded-lg overflow-hidden">
                  ...
                  <button onClick={stopScanner} className="w-full bg-red-100 text-red-600 py-2 font-medium hover:bg-red-200 transition">إلغاء المسح</button>
                </div>
              )}
            </div>
            <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-4">
                {image ? (
                  ...
                  <div className="w-20 h-20 bg-gray-100 rounded-lg border flex items-center justify-center text-gray-400"><ImageIcon size={32} /></div>
                )}
                <label className="cursor-pointer bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition">
                  اختر صورة
                  <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                </label>
                {image && <button onClick={() => { setImage(''); setImageFile(null); }} className="text-red-500 hover:text-red-700 text-sm">إزالة الصورة</button>}
              </div>
            </div>
          </div>
          <div className="flex gap-3 pt-6 mt-4 border-t border-gray-100">
            <button onClick={handleSave} disabled={!name.trim() || loading} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50 transition">
              {loading ? 'جاري الحفظ...' : 'حفظ'}
            </button>
            <button onClick={resetForm} className="bg-gray-200 text-gray-800 px-6 py-2 rounded-lg hover:bg-gray-300 transition">إلغاء</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          <div key={product.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="space-y-1 text-sm text-gray-600 mb-4 flex-1">
                <div className="flex justify-between"><span>سعر الجملة:</span><span className="font-medium">{product.wholesalePrice ? `${product.wholesalePrice} د.ج` : '-'}</span></div>
                <div className="flex justify-between"><span>سعر التفصيل:</span><span className="font-medium text-green-600">{product.retailPrice ? `${product.retailPrice} د.ج` : '-'}</span></div>
                <div className="flex justify-between"><span>الباركود:</span><span className="font-mono text-xs">{product.barcode || '-'}</span></div>
              </div>
              <div className="flex gap-2 pt-3 border-t border-gray-100">
                <button onClick={() => handleEdit(product)} className="flex-1 py-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition flex justify-center items-center gap-2">
                  <Edit size={16} /> تعديل
                </button>
                <button onClick={() => handleDelete(product.id!)} className="p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
        ...
        {products.length === 0 && !isAdding && (
          <div className="col-span-full text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
            ...
            <button onClick={() => setIsAdding(true)} className="mt-4 text-blue-600 font-medium hover:underline">أضف سلعتك الأولى</button>
          </div>
        )}
      </div>
    </div>
